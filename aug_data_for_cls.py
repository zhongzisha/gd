...
if __name__ == '__main__':
    src_dir = 'E:/patches/train1/pos'
    dst_dir = 'E:/patches/train1_aug/pos'
    ...
    for augi in range(aug_times):
        for i in range(len(files)):
            images[0] = cv2.imread(files[i])[:,:,::-1] # RGB
            images_aug = seq(images=images)
            cv2.imwrite(os.path.join(dst_dir, files[i].split(os.sep)[-1].replace('.jpg', '_aug%d.jpg'%(augi))),
                        images_aug[0][:,:,::-1])
