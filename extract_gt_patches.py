...
def main(subset='train'):
    ...
    for ti in range(len(tiffiles)):
        ...
        save_sub_dirs = ['large', 'median', 'small']
        ...
        if True:
            ...
            for j, (a, label) in enumerate(zip(b, labels)):  # per item
                if label == 1:
                    ...
                    #     save_filename = '%s/small/%d.jpg' % (save_dir, j)
                    # elif 80 <= width <= 200 or 80 <= height <= 200:
                    #     save_filename = '%s/median/%d.jpg' % (save_dir, j)
                    # else:
                    #     save_filename = '%s/large/%d.jpg' % (save_dir, j)
                    save_filename = '%s/%d.jpg' % (save_dir, j)
