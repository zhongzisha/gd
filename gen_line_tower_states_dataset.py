...
def add_line(im, mask, p0s, p1s, line_width=None):
...
def paste_fg_images_to_bg(im, mask, fg_images, count=10):
    ...
    for step in range(count):
        ...
        h, w = bgr.shape[:2]
        print(w, h)
        short_len = np.random.randint(50, 80)
        if w < h:
            scale = short_len / w
            w = short_len
            h = int(scale * h)
        ...
        # w, h = np.random.randint(50, 80), np.random.randint(50, 80)
...
def generate_test_images():
    if os.name == 'nt':
        source = 'G:/gddata/all'
        gt_dir = 'G:\\gddata\\all'
        save_root = 'E:\\generated_big_test_images\\tower_states'
        ...
        save_root = '/media/ubuntu/Data/generated_big_test_images/tower_states'
        foreign_fg_dir = '/media/ubuntu/Data/line_foreign_object_detection/fg_images'
        tower_fg_root = '/media/ubuntu/Data/ganta_patch_classification/ganta_patch_classification/val'
    ...
    towers_states = ['normal', 'jieduan', 'wanzhe']  #   'nontower' is removed here
    foreign_fg_filenames = []  # glob.glob(foreign_fg_dir + '/*.png')
    for tower_state in towers_states:
        tmp_names = glob.glob(os.path.join(tower_fg_root, tower_state, '*.jpg'))
        indices = np.arange(len(tmp_names))
        count = min(100, len(tmp_names))
        selected_indices = np.random.choice(indices, size=count, replace=False)
        foreign_fg_filenames += [tmp_names[ind] for ind in selected_indices]
    ...
    for tiffile in tiffiles:
        ...
        save_tif_filename = os.path.join(save_root, file_prefix + '.tif')
        ...
        for batch_idx, img_filename in enumerate(img_filenames):
            ...
            if os.path.exists(save_tif_filename):
                ...
                continue
            im_sub = np.array(Image.open(img_filename))
            mask = np.array(Image.open(os.path.join(save_masks_dir, file_prefix1 + '.tif')).convert('L'))
            if np.max(mask) == 0:
            ...
            if abs(x1 - x2) == 1 and (1 < x1 < W - 1):
                ...
                im_sub, mask = add_line(im_sub, mask, p0s, p1s, line_width=1)
            ...
            fg_images = []
            total_count = np.random.randint(5, 10)
            fg_count = np.random.randint(low=1, high=2)
            while True:
                random_filenames = np.random.choice(foreign_fg_filenames,
                                                    size=total_count,
                                                    replace=True)
                for name in random_filenames:
                    ...
                    fg_im = cv2.imread(name, cv2.IMREAD_UNCHANGED)  # BGRA
                    if fg_im.shape[2] == 3:
                        fg_im = np.concatenate([
                                                   fg_im,
                                                   255 * np.ones((fg_im.shape[0],
                                                                  fg_im.shape[1], 1),
                                                                 dtype=np.uint8)],
                                               axis=2)
                    if fg_im.shape[2] != 4:
                ...
                if len(fg_images) > 1:
                    break
            print('num_fg_images: ', len(fg_images))
            print('paste the fg_image to im_sub')
            im_sub1, im_sub1_com, im_sub1_blend, mask1, boxes1, boxes1_mask = \
                paste_fg_images_to_bg(im_sub, mask, fg_images, fg_count)
